...
export module data {
    export default class SemanticQueryRewriter {
        ...
        public rewriteWhere(whereItems: SQFilter[], from: SQFrom): SQFilter[] {
            ...
            for (var i = 0, len = whereItems.length; i < len; i++) {
                ...
                if (originalWhere.target) updatedWhere.target = _.map(originalWhere.target, (e) => SQExprRewriterWithSourceRenames.rewrite(e.accept(this.exprRewriter), from));
