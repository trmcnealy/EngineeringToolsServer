...
export module data {
     ...
     */
    export default class SemanticQuery {
        ...
        private static createWithTrimmedFrom(from: SQFrom, where: SQFilter[], orderBy: SQSortDefinition[], select: NamedSQExpr[]): SemanticQuery {
        ...
        select(): ArrayExtensions.ArrayNamedItems<NamedSQExpr>;
        ...
        private getSelect(): ArrayExtensions.ArrayNamedItems<NamedSQExpr> {
            return ArrayExtensions.extendWithName<NamedSQExpr>(
                _.map(this.selectItems, (s) => {
                    return {
                        name: s.name,
                        expr: s.expr
                    };
        ...
        public removeSelect(expr: SQExpr): SemanticQuery {
            ...
            for (var i = 0, len = originalItems.length; i < len; i++) {
                ...
                if (SQExpr.equals(originalExpr.expr, expr)) continue;
        ...
        public removeOrderBy(expr: SQExpr): SemanticQuery {
            ...
            for (var i = sorts.length - 1; i >= 0; i--) {
                if (SQExpr.equals(sorts[i].expr, expr)) sorts.splice(i, 1);
            }
        ...
        public selectNameOf(expr: SQExpr): string {
            var index = SqExprUtils.indexOfExpr(
                _.map(this.selectItems, (s) => s.expr),
                expr
            );
        ...
        public addWhere(filter: SemanticFilter): SemanticQuery {
            ...
            for (var i = 0, len = incomingWhere.length; i < len; i++) {
                ...
                if (clause.target) updatedClause.target = _.map(clause.target, (t) => SQExprRewriterWithSourceRenames.rewrite(t, from));
    ...
    export default class SemanticFilter {
        private fromValue: SQFrom;
        private whereItems: SQFilter[];
    ...
    export default class SQFrom {
        ...
        public ensureEntity(entity: SQFromEntitySource, desiredVariableName?: string): QueryFromEnsureEntityResult {
            ...
            for (var i = 0, len = keys.length; i < len; i++) {
                var key = keys[i],
                    item = this.items[key];
                if (item && entity.entity === item.entity && entity.schema === item.schema) return {name: key};
            }
    ...
    export default class SQExprRewriterWithSourceRenames extends SQExprRewriter {
        ...
        public visitEntity(expr: SQEntityExpr): SQExpr {
            ...
            if (updatedName) return new SQEntityExpr(expr.schema, expr.entity, updatedName);
