...
export module data {
    ...
    export module DataViewObjectDefinitions {
        ...
        export function ensure(defns: DataViewObjectDefinitions, objectName: string, selector: Selector): DataViewObjectDefinition {
            ...
            for (var i = 0, len = defnsForObject.length; i < len; i++) {
                ...
                if (Selector.equals(defn.selector, selector)) return defn;
            }
        ...
        export function deleteProperty(defns: DataViewObjectDefinitions, objectName: string, selector: Selector, propertyName: string): void {
            //debug.assertValue(defns, 'defns');
        ...
        export function getValue(defns: DataViewObjectDefinitions, propertyId: DataViewObjectPropertyIdentifier, selector: Selector): DataViewObjectPropertyDefinition {
        ...
        export function getPropertyContainer(
            ...
            for (var i = 0, len = defnsForObject.length; i < len; i++) {
                var defn = defnsForObject[i];
                if (Selector.equals(defn.selector, selector)) return defn.properties;
            }
        }

        export function propertiesAreEqual(currentObject: DataViewObjectPropertyDefinitions, modifiedObject: DataViewObjectPropertyDefinitions): boolean {
            return JsonComparer.equals(currentObject, modifiedObject);
        }
    }
}
