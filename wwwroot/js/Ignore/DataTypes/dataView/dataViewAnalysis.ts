...
export module DataViewAnalysis {
    import QueryProjectionByProperty = data.QueryProjectionsByRole;
    ...
    export function validateAndReshape(dataView: DataView, dataViewMappings: DataViewMapping[]): ValidateAndReshapeResult {
        if (!dataViewMappings || dataViewMappings.length === 0) return {dataView: dataView, isValid: true};

        if (dataView) {
            var dataViewMapping = dataViewMappings[0];

            // Keep the original when possible.
            if (supports(dataView, dataViewMapping)) return {dataView: dataView, isValid: true};
    ...
    function reshapeCategorical(dataView: DataView, dataViewMapping: DataViewMapping): ValidateAndReshapeResult {
        ...
        if (!categorical) return {isValid: false};
        ...
        if (supportsCategorical(dataView, dataViewMapping)) return {dataView: dataView, isValid: true};
    ...
    function reshapeSingle(dataView: DataView, singleRoleMapping: DataViewSingleMapping): ValidateAndReshapeResult {
        ...
        if (dataView.single) return {dataView: dataView, isValid: true};
    ...
    function reshapeTable(dataView: DataView, tableRoleMapping: DataViewTableMapping): ValidateAndReshapeResult {
        ...
        if (dataView.table) return {dataView: dataView, isValid: true};
    ...
    function supportsCategorical(dataView: DataView, categoryRoleMapping: DataViewCategoricalMapping, usePreferredDataViewSchema?: boolean): boolean {
        ...
        if (categoryRoleMapping.rowCount) {
            ...
            if (rowCount) {
                var len: number = 0;
                if (dataViewCategorical.values && dataViewCategorical.values.length) len = dataViewCategorical.values[0].values.length;
                else if (dataViewCategorical.categories && dataViewCategorical.categories.length) len = dataViewCategorical.categories[0].values.length;
    ...
    export function conforms(value: number, range: NumberRange, ignoreMin?: boolean): boolean {
        ...
        if (!ignoreMin && range.min !== undefined && range.min > value) return false;
    ...
    export function getPropertyCount(roleName: string, projections: QueryProjectionByProperty, useActiveIfAvailable?: boolean): number {
        ...
        if (projectionsForRole) {
            if (useActiveIfAvailable) return 1;
        ...
        if (dataView1 && dataView2 && dataView1.categorical && dataView2.categorical) {
            ...
            if (dv1Categories && dv2Categories && dv1Categories.length === dv2Categories.length) {
                for (var i = 0, len = dv1Categories.length; i < len; i++) {
                    ...
                    if (dv1Length !== getLengthOptional(dv2Identity)) return false;

                    for (var j = 0; j < dv1Length; j++) {
                        if (!DataViewScopeIdentity.equals(dv1Identity[j], dv2Identity[j])) return false;
                    }
                }
    ...
    function getLengthOptional(identity: DataViewScopeIdentity[]): number {
        if (identity) return identity.length;
    ...
    export function areMetadataColumnsEquivalent(column1: DataViewMetadataColumn, column2: DataViewMetadataColumn): boolean {
        ...
        if (column1.displayName !== column2.displayName) return false;

        if (column1.queryName !== column2.queryName) return false;

        if (column1.isMeasure !== column2.isMeasure) return false;

        if (column1.type !== column2.type) return false;
    ...
    export function isMetadataEquivalent(metadata1: DataViewMetadata, metadata2: DataViewMetadata) {
        ...
        if (previousColumnsLength !== newColumnsLength) return false;

        for (var i: number = 0; i < newColumnsLength; i++) {
            if (!DataViewAnalysis.areMetadataColumnsEquivalent(metadata1.columns[i], metadata2.columns[i])) return false;
        }
