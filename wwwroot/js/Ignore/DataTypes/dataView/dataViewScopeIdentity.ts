...
export module DataViewScopeIdentity {
    ...
    export function filterFromIdentity(identities: DataViewScopeIdentity[], isNot?: boolean): data.SemanticFilter {
        ...
        for (var i = 0, len = identities.length; i < len; i++) {
            ...
            expr = expr ? data.SQExprBuilder.or(expr, identity.expr) : identity.expr;
        }

        if (expr && isNot) expr = data.SQExprBuilder.not(expr);

        return data.SemanticFilter.fromSQExpr(expr);
    }
}
