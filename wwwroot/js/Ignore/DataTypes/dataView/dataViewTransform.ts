...
export module data {
    ...
    export module DataViewTransform {
        ...
        function transformEmptyDataView(
            objectDescriptors: DataViewObjectDescriptors,
            transforms: DataViewTransformActions,
            colorAllocatorFactory: IColorAllocatorFactory
        ): DataView[] {
        ...
        function transformDataView(
            ...
            transforms: DataViewTransformActions,
            colorAllocatorFactory: IColorAllocatorFactory,
            selectsToInclude?: INumberDictionary<boolean>
        ): DataView {
        ...
        function transformSelects(
            ...
            if (dataView.matrix) {
                ...
                if (shouldPivotMatrix(dataView.matrix, roleMappings)) DataViewPivotMatrix.apply(dataView.matrix, matrixTransformationContext);
            }
        ...
        function applyTransformsToColumns(
            ...
            selects: DataViewSelectTransform[],
            rewrites: ValueRewrite<DataViewMetadataColumn>[]
            ...
            for (var propertyKey in settings) {
                ...
                switch (propertyKey) {
                    ...
                    case "imageVisualSettings":
                        ...
                        if (propertyValue && propertyValue.imageUrl) upgradedPropertyValue = SQExprBuilder.text(propertyValue.imageUrl);
                        break;
        ...
        function setObjectDefinition(objects: DataViewObjectDefinitions, objectName: string, propertyName: string, value: any): void {
            ...
            if (object === undefined) object = objectContainer[0] = {properties: {}};

            object.properties[propertyName] = value;
        }

        function applyRewritesToCategorical(
            prototype: DataViewCategorical,
        ...
        function applyRewritesToTable(
            ...
            for (var i = 0, len = keys.length; i < len; i++) {
                ...
                if (i === columns.length) columns.push(sourceColumn);
                else {
            ...
            // Reorder the rows
            var rows = Prototype.overrideArray(table.rows, (row: any[]) => {
                var newRow: any[] = [];
        ...
        function createTableColumnPositionMapping(
            ...
            if (ArrayExtensions.sequenceEqual(originalOrder, newOrder, (x: number, y: number) => x === y)) return;
        ...
        function applyRewritesToMatrix(
            ...
            roleMappings: DataViewMapping[],
            projectionOrdering: DataViewProjectionOrdering,
            context: MatrixTransformationContext
        ): DataViewMatrix {
            //debug.assertValue(prototype, 'prototype');
            //debug.assertValue(columnRewrites, 'columnRewrites');
            ...
            function overrideHierarchy(hierarchy: DataViewHierarchy): DataViewHierarchy {
                ...
                var newLevels = Prototype.overrideArray(hierarchy.levels, (level: DataViewHierarchyLevel) => {
                    var newLevel: DataViewHierarchyLevel = null;
                    var levelSources = Prototype.overrideArray(level.sources, override);
                    if (levelSources) newLevel = ensureRewritten<DataViewHierarchyLevel>(newLevel, level, (h) => (h.sources = levelSources));

                    return newLevel;
                });
                if (newLevels) rewrittenHierarchy = ensureRewritten<DataViewHierarchy>(rewrittenHierarchy, hierarchy, (r) => (r.levels = newLevels));
                ...
                if (projectionOrdering && valueSources.length > 1) {
                    ...
                    if (columnLevels > 0) {
                        ...
                        if (newToOldPositions) {
                            ...
                            if (columnLevels === 1) reorderChildNodes(columns.root, newToOldPositions);
                            else forEachNodeAtLevel(columns.root, columnLevels - 2, (node: DataViewMatrixNode) => reorderChildNodes(node, newToOldPositions));
        ...
        function reorderChildNodes(node: DataViewMatrixNode, newToOldPositions: NumberToNumberMapping): void {
            ...
            for (var i = 0, len = numKeys; i < len; i++) {
                ...
                // In the case we've hit the end of our columns array, but still have position reordering keys,
                // there is a duplicate column so we will need to add a new column for the duplicate data
                if (i === children.length) children.push(sourceColumn);
                else {
        ...
        function transformObjects(
            ...
            if (selectTransforms) DataViewObjectEvaluationUtils.addDefaultFormatString(objectsForAllSelectors, objectDescriptors, dataView.metadata.columns, selectTransforms);
        ...
        function createRuleEvaluationInstance(
            ...
            ruleType: StructuralTypeDescriptor
        ): DataViewObjectDefinitionsForSelectorWithRule {
            //debug.assertValue(dataView, 'dataView');
            //debug.assertValue(colorAllocatorFactory, 'colorAllocatorFactory');
            ...
            if (ruleType.filter) return createRuleEvaluationInstanceFilter(dataView, ruleDesc, selectorToCreate, objectName, <SemanticFilter>propertyValue);
        }
        ...
        function createColorAllocatorLinearGradient3(
            ...
            if (linearGradient3.min.value === undefined || linearGradient3.mid.value === undefined || linearGradient3.max.value === undefined) {
                ...
                var splitScales: boolean = linearGradient3.min.value === undefined && linearGradient3.max.value === undefined && linearGradient3.mid.value !== undefined;
        ...
        function evaluateDataRepetition(
            ...
            if (dataViewCategorical) {
                ...
                selectorMatched =
                    evaluateDataRepetitionCategoricalCategory(dataViewCategorical, objectDescriptors, selector, rules, containsWildcard, objectDefns) || selectorMatched;

                // 2) Match against valueGrouping
                selectorMatched =
                    evaluateDataRepetitionCategoricalValueGrouping(dataViewCategorical, objectDescriptors, selector, rules, containsWildcard, objectDefns) || selectorMatched;
        ...
        function matchRulesToDataViewCategorical(rules: RuleEvaluation[], dataViewCategorical: DataViewCategorical): {rule: RuleEvaluation; inputValues: any[]}[] {
        ...
        function evaluateDataRepetitionCategoricalValueGrouping(
            ...
            if (!Selector.matchesKeys(selector, [dataViewCategoricalValues.identityFields])) return;
        ...
        function findSelectedCategoricalColumn(dataViewCategorical: DataViewCategorical, selector: Selector) {
            ...
            if (!Selector.matchesKeys(selector, [categoricalColumn.identityFields])) return;
        ...
        export function createTransformActions(
            ...
            objectDefns: DataViewObjectDefinitions
        ): DataViewTransformActions {
                    ...
                    for (var i = 0, len = querySelects.length; i < len; i++) {
                        ...
                        if (selectTransform.format && objectDescs) {
                            ...
                            if (formatStringProp) {
                                ...
                                DataViewObjectDefinitions.ensure(objectDefns, formatStringProp.objectName, {metadata: selectTransform.queryName}).properties[
                                    formatStringProp.propertyName
                                ] = SQExprBuilder.text(selectTransform.format);
                            }
                        }
                    }
                }
            ...
            if (visualElements) {
                ...
                for (var i = 0; i < visualElementsLength; i++) {
                    ...
                    if (visualElement.DataRoles) {
                        ...
                        if (!transforms.projectionOrdering) transforms.projectionOrdering = {};
        ...
        function toTransformSelect(select: SelectMetadata, index: number): DataViewSelectTransform {
            ...
            if (select.Name) result.queryName = select.Name;
            else if (!result.queryName) result.queryName = "$select" + index;
        ...
        function populateDataRoles(roles: DataRole[], selects: DataViewSelectTransform[], projectionOrdering: DataViewProjectionOrdering): void {
            ...
            for (var i = 0, len = roles.length; i < len; i++) {
                ...
                if (projectionOrderingForRole === undefined) projectionOrderingForRole = projectionOrdering[roleName] = [];
        ...
        export function createValueColumns(values: DataViewValueColumn[] = [], valueIdentityFields?: SQExpr[], source?: DataViewMetadataColumn): DataViewValueColumns {
        ...
        function setGrouped(values: DataViewValueColumns, groupedResult?: DataViewValueColumnGroup[]): void {
            values.grouped = groupedResult ? () => groupedResult : () => groupValues(values);
        }
            ...
            for (var i = 0, len = values.length; i < len; i++) {
                ...
                if (!currentGroup || currentGroup.identity !== value.identity) {
                    ...
                    if (value.identity) {
                        ...
                        if (source.groupName !== undefined) currentGroup.name = source.groupName;
                        else if (source.displayName) currentGroup.name = source.displayName;
                    }
        ...
        function determineCategoricalTransformation(categorical: DataViewCategorical, dataViewMappings: DataViewMapping[]): CategoricalDataViewTransformation {
            ...
            if (values.grouped().some((vg) => !!vg.identity)) return;
            ...
            for (var i = 0, len = dataViewMappings.length; i < len; i++) {
                ...
                if (!hasRolesGrouped(categoryRoles, <DataViewGroupedRoleMapping>roleMappingCategorical.values)) continue;
                ...
                if (hasCategoryRole) return CategoricalDataViewTransformation.SelfCrossJoin;
        ...
        function shouldPivotMatrix(matrix: DataViewMatrix, dataViewMappings: DataViewMapping[]): boolean {
            ...
            for (var i = 0, len = dataViewMappings.length; i < len; i++) {
                ...
                if (
                    !hasRolesFor(rowRoles, <DataViewRoleForMappingWithReduction>roleMappingMatrix.rows) &&
