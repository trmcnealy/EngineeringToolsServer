...
export module data {
    export module DataViewPivotMatrix {
        ...
        export function apply(dataViewMatrix: DataViewMatrix, context: MatrixTransformationContext): void {
            ...
            if (!context.columnHierarchyRewritten) dataViewMatrix.columns = Prototype.inherit(dataViewMatrix.columns);
            var columns = dataViewMatrix.columns;

            if (!context.rowHierarchyRewritten) dataViewMatrix.rows = Prototype.inherit(dataViewMatrix.rows);
